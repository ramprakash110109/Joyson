...
<?php
	include('../connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM kcosting_table WHERE costing_id= :userid");
...
<form action="saveeditknitted.php" method="post" enctype="multipart/form-data">
<input type="hidden" name="memi" value="<?php echo $id; ?>" />
<center><h4><i class="icon-edit icon-large"></i> Edit Knitted Costing </h4></center>
<hr>
...
<span> Description </span><input type="text" list="desc" style="width:265px; height:30px;" name="desc"value="<?php echo $row['description'];?>" ><br>
<datalist id="desc">
    <option value="T Shirt">
    <option value="Shorts">
    <option value="Pant">
    <option value="Skirt">
    <option value="T Shirt with overcoat">
  </datalist>

<span> Size </span><input type="text" style="width:265px; height:30px;" name="size" value="<?php echo $row['size'];?>" ><br>


<span> Shirting</span><select id="fname1" name='fab[name][]'  style='width:265px; id='fabn"+i+"' height:30px; margin-left:-5px;' Required>
<?php include('../connect.php');
		 $result = $db->prepare("SELECT * FROM fabric_table where type='knitted'");
		 $result->execute(); ?>
<option value="<?php echo $row['shirting'];?>"><?php echo getFabName($row['shirting']); ?></option>
<?php for($i=0; $row1 = $result->fetch(); $i++){ ?>
<option value='<?php echo $row1['f_id'];?>'> <?php echo $row1['f_description']."-".$row1['mill_name']."-".$row1['design_no']; ?></option>
<?php }?>
</select><br>
<span> Shirting qty(grams)</span> <input id="fqty1" type='number' class="fabq" min='0.0' step='0.1' style='width:265px; height:30px;' name="fab[qty][]" value="<?php echo $row['shirting_qty'];?>" Required><br> <br>
...
<span> Suiting qty(grams)</span> <input id='fqty2' type='number' class="fabq"  min='0.0' step='0.1' style='width:265px; height:30px;' name="fab[qty][]" value="<?php echo $row['suiting_qty'];?>" Required><br> <br> <br>
<!-- to display trims -->
<?php
...
<span> Power  table charges </span><input id="ptc" class="excost" type="number"  min="0.00" step="0.01" style="width:265px; height:30px;" name="power_table" placeholder="0.00" value="<?php echo $row['power_table'];?>"><br>
<span> Printing charges </span><input id="pc" class="excost" type="number"  min="0.00" step="0.01" style="width:265px; height:30px;" name="printing" placeholder="0.00" value="<?php echo $row['printing'];?>"><br>
<span> Embroidery charges </span><input id="ec" class="excost" type="number"  min="0.00" step="0.01" style="width:265px; height:30px;" name="embroidery" placeholder="0.00" value="<?php echo $row['embroidery'];?>"><br>
<span> Actual price </span><input type="number" style="width:265px; height:30px;" id="oc" name="ocost"  placeholder="0.00" value="<?php echo $row['price'];?>" readonly="true"><br>
<span> Profit (%) </span><input type="number" list="profit" id="gain" style="width:265px; height:30px;" name="profit" min="0" step="5" max="50"  placeholder="%" value="<?php echo $row['profit'];?>">

<br>
<span> Rate/piece </span><input type="number" id="rp" style="width:265px; height:30px;" name="rate"  placeholder="0.00" value="<?php echo $row['cost'];?>" readonly="true"><br>
