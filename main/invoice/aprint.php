...
else
{
	...
	{
		$result = $db->prepare("SELECT * FROM accessories where acc_id='".$costing[$j]."'");
		$result->execute();
		$row = $result->fetch();
		$cp=((float)$row['price']+(float)$row['freight'])*(float)$item_qty_set[$j];
		$sp=(float)$row['cost']*(float)$item_qty_set[$j];
		$profit[$j]=(float)$sp-$cp;
		$fab1stk=getStock($costing[$j],'accessories','acc_id');
		$sql = "UPDATE accessories SET stock='".((float)$fab1stk-(float)$item_qty_set[$j])."' WHERE acc_id=? and stock not like '0';";
		$q = $db->prepare($sql);
		$q->execute(array($costing[$j]));
