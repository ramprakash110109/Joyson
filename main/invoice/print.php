...
else
{
	...
	{
		$result = $db->prepare("SELECT * FROM costing_table where costing_id='".$costing[$j]."'");
		$result->execute();
		$row = $result->fetch();
		...
		$sql = "UPDATE fabric_table SET stock='".((float)$fab1stk-(float)$obtainfab1stk)."' WHERE f_id=? and type='woven' and stock not like '0';";
		$q = $db->prepare($sql);
		...
		$sql = "UPDATE fabric_table SET stock='".((float)$fab2stk-(float)$obtainfab2stk)."' WHERE f_id=? and type='woven' and stock not like '0';";
		$q = $db->prepare($sql);
	...
	{
		$salesprofit+=$profit[$l];
	}
	$sql = "UPDATE invoice SET profit='".$salesprofit."' WHERE invoice_no=?;";
	$q = $db->prepare($sql);
